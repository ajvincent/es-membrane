import {
  TrackingCollections
} from "../../source/mockups/TrackingCollections.js";
...
>
{
  readonly #CLASS_PROPERTIES = TrackingCollections.TrackPrivateFields(
    this, OneToOneStrongMap, new TrackingCollections.PrivateClassPropertiesMap<
      "#baseMap" | "#weakValueToInternalKeyMap"
    >
  );

  get #baseMap(): WeakStrongMap<InternalKey, StrongKeyType, ValueType> {
    return this.#CLASS_PROPERTIES.castGet<WeakStrongMap<InternalKey, StrongKeyType, ValueType>, true>("#baseMap");
  }
  set #baseMap(value: WeakStrongMap<InternalKey, StrongKeyType, ValueType>) {
    this.#CLASS_PROPERTIES.set("#baseMap", value);
  }

  get #weakValueToInternalKeyMap(): WeakMap<ValueType, InternalKey> {
    return this.#CLASS_PROPERTIES.castGet<WeakMap<ValueType, InternalKey>, true>("#weakValueToInternalKeyMap")
  }
  set #weakValueToInternalKeyMap(value: WeakMap<ValueType, InternalKey>) {
    this.#CLASS_PROPERTIES.set("#weakValueToInternalKeyMap", value);
  }

  public autoCleanup = false;

  constructor() {
    this.#baseMap = new WeakStrongMap;
    this.#weakValueToInternalKeyMap = new TrackingCollections.WeakMap;
  }
  ...
  ) : boolean
  {
    ...
    if (__returnValue__) {
      ...
      // autoCleanup is for testing reference holding.
      if (this.autoCleanup) {

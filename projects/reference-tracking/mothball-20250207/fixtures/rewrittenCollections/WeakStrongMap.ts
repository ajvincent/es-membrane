...
type RootWeakMap<
  ...
  StrongKeyType,
  ValueType
> = WeakMap<WeakKeyType, Map<StrongKeyType, ValueType>>;
...
>
{
  readonly #CLASS_PROPERTIES = TrackingCollections.TrackPrivateFields(
    this, WeakStrongMap, new TrackingCollections.PrivateClassPropertiesMap<"#root">
  );

  get #root(): RootWeakMap<WeakKeyType, StrongKeyType, ValueType>
  {
    return this.#CLASS_PROPERTIES.castGet<
      RootWeakMap<WeakKeyType, StrongKeyType, ValueType>, true
    >("#root");
  }
  ...
  {
    this.#CLASS_PROPERTIES.set("#root", new TrackingCollections.WeakMap<WeakKeyType, Map<StrongKeyType, ValueType>>);
    if (iterable) {
  ...
  {
    const innerMap = this.#root.get(weakKey);
    return new TrackingCollections.Set(innerMap?.keys() ?? []);
  }
  ...
  ): this
  {
    if (!this.#root.has(weakKey)) {
      this.#root.set(weakKey, new TrackingCollections.Map);
    }

...
it("ReplaceableValueMap provides a matching replacement for each value it receives", () => {
  ...
  const { source: fooReplaced_0, userContext: bar_0 } = replacer.getOrInsertComputed(fooOriginal_0, buildReplacement);
  ...
  const { source: fooReplaced_1, userContext: bar_1} = replacer.getOrInsertComputed(fooOriginal_0, buildReplacement);
  expect(fooReplaced_1).toBe(fooReplaced_0);
  expect(bar_1).toBe(bar_0);
  expect(replacerArguments).toEqual([false, fooOriginal_0, bar_0]);

  const { source: fooReplaced_2, userContext: bar_2} = replacer.getOrInsertComputed(fooReplaced_0, buildReplacement);
  expect(fooReplaced_2).toBe(fooReplaced_0);
  expect(bar_2).toBe(bar_0);
  expect(replacerArguments).toEqual([false, fooOriginal_0, bar_0]);

  const fooOriginal_3 = new Foo;
  const {source: fooReplaced_3, userContext: bar_3} = replacer.getOrInsertComputed(fooOriginal_3, buildReplacement);

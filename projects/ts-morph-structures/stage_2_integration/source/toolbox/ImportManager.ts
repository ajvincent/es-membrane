...
export default class ImportManager
{
  ...
  {
    if (declStructure.defaultImport && this.#importedKeyToDeclMap.has(declStructure.defaultImport))
    ...
    if (alreadyDefinedNames.length > 0) {
      throw new Error("These names are already in the manager: " + JSON.stringify(alreadyDefinedNames));
    }
  ...
  public addImports(context: AddImportContext): void {
    const { isPackageImport, isDefaultImport, importNames } =
      context;
    ...
    if (alreadyDefined.length) {
      throw new Error(`These names are already in the manager in other declarations: ${
        JSON.stringify(alreadyDefined)
  ...
  {
    const decl: ImportDeclarationImpl | undefined = this.#importedKeyToDeclMap.get(name);
    if (!decl)
      return undefined;

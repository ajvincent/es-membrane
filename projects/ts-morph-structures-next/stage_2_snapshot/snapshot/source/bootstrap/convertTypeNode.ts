...
export default function convertTypeNode(
  ...
  */
  ...
  if (Node.isOptionalTypeNode(typeNode)) {
    const childTypeNode = convertTypeNode(
      typeNode.getTypeNode(),
      consoleTrap,
      subStructureResolver,
    );
    if (!childTypeNode) return null;
...
function convertImportTypeNode(
  ...
  const attrNodes: ImportAttributes | undefined =
    importTypeNode.getAttributes();
  let attrs: ImportAttributeImpl[];
  if (attrNodes) {
    const elementNodes: ImportAttribute[] = attrNodes.getElements();
    attrs = elementNodes.map((node) =>
      subStructureResolver(node),
    ) as ImportAttributeImpl[];
  } else {
    attrs = [];
  }
  ...
  let qualifier:
    ...
    | null;
  if (qualifierNode) {
    const qStructure: string | QualifiedNameTypeStructureImpl =
      composeQualifiedName(qualifierNode);
    if (typeof qStructure === "string")
      qualifier = LiteralTypeStructureImpl.get(qStructure);
    else qualifier = qStructure;
  } else {
    qualifier = null;
  }

  const importTypeStructure = new ImportTypeStructureImpl(
    argStructure,
    attrs,
    qualifier,
    [],

...
export type IsEmptyArray<Elements extends ReadonlyArray<unknown>> =
  [] extends Elements ? true : false;

export type HasHead<Elements extends ReadonlyArray<unknown>> =
  [] extends Elements ? false : true;

export type HasTail<Elements extends ReadonlyArray<unknown>> =
  [] extends Elements ? false :
  Elements extends [unknown] ? false :
  true;

export type Head<Elements extends ReadonlyArray<T>, T = unknown> =
  HasHead<Elements> extends true ? ShiftArrayElement<Elements, T>["element"] : never;

export type Tail<Elements extends ReadonlyArray<T>, T = unknown> =

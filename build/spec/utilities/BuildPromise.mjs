...
describe("BuildPromise.mts: ", () => {
    ...
    function createSpyPromise(willResolve, value) {
    ...
    describe(`Without starting from main, BuildPromiseSet.get("foo")`, () => {
        ...
        it(".description can be set once", () => {
            ...
            expect(() => foo.description = "wop").toThrowError(`Description already set for target "foo"`);
            expect(foo.description).toBe("bar");
        });
        describe(`.addSubtarget()`, () => {
            it(`called with "bar" once adds a subtarget`, () => {
                expect(() => foo.addSubtarget("bar")).not.toThrow();
            ...
            it(`called with "foo" throws`, () => {
                expect(() => foo.addSubtarget("foo")).toThrowError("Cannot include this as its own subtarget");
            });
        ...
        describe(".addTask()", () => {
            ...
            it("will throw if bpSet.markReady() has been called", () => {
                const spy = jasmine.createSpy();
                bpSet.markReady();
                expect(() => foo.addTask(spy)).toThrowError("Build step has started");
            });
        ...
        it(".run() throws", async () => {
            await expectAsync(foo.run()).toBeRejectedWithError("Build promises are not running!");
    ...
    describe("Without starting, BuildPromiseSet.main", () => {
        ...
        describe(".addSubtarget()", () => {
            it("cannot be called until we call bpSet.markReady()", () => {
                expect(() => main.addSubtarget("foo")).toThrowError("Cannot attach targets to main target until we are ready (call BuildPromiseSet.markReady())");
                bpSet.get("foo");
                bpSet.markReady();
                expect(() => main.addSubtarget("foo")).not.toThrow();
                expect(main.deepTargets).toEqual(["foo"]);
            });
            it("throws with an unknown target", () => {
                expect(() => main.addSubtarget("foo")).toThrowError("Cannot attach targets to main target until we are ready (call BuildPromiseSet.markReady())");
                bpSet.markReady();
                expect(() => main.addSubtarget("foo")).toThrowError(`Cannot add an undefined target "foo" to main!`);
                expect(main.deepTargets).toEqual([]);
            });
        });
    });
    describe("Without starting, BuildPromiseSet.main.addTask()", () => {
        ...
        it("will throw if bpSet.markReady() has been called", () => {
            const spy = jasmine.createSpy();
            bpSet.markReady();
            expect(() => main.addTask(spy)).toThrowError("Build step has started");
        });
        it("will throw if bpSet.markClosed() has been called", () => {
            const spy = jasmine.createSpy();
            bpSet.markClosed();
    ...
    describe("BuildPromiseSet.main.run()", () => {
        ...
        it("throws before bpSet.markReady() is called", async () => {
            await expectAsync(main.run()).toBeRejectedWithError("Build promises are not running!");
        });
        ...
        it("runs a full tree of tasks, until there is a failure", async () => {
            ...
            bpSet.main.addSubtarget("alpha");
            bpSet.main.addSubtarget("beta");
            await expectAsync(bpSet.main.run()).toBeRejectedWith(errObject);
            expect(task1).toHaveBeenCalledOnceWith();
            expect(task2).toHaveBeenCalledOnceWith();
            expect(task3).toHaveBeenCalledOnceWith();
